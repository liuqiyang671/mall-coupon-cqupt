...
/**
 * RocketMQ delay-message integration test with bounded waiting instead of infinite blocking.
 */
@Slf4j
@Tag("integration")
@SpringBootTest
public final class RocketMQ5xDelayProducerConsumerIT {
    ...
    public void producerSendTest() {
        ...
        Message message = new Message("EngineTestDelayTopic", "aaa".getBytes(StandardCharsets.UTF_8));
        message.setDeliverTimeMs(remindTime.getTime());
        DefaultMQProducer defaultMQProducer = rocketMQTemplate.getProducer();
        SendResult sendResult = defaultMQProducer.send(message);
        log.info("Delay message send result: {}, sent at: {}", sendResult, DateUtil.formatTime(new Date()));

        Assertions.assertTrue(
                messageConsumedLatch.await(20, TimeUnit.SECONDS),
                "RocketMQ delay message was not consumed in time"
        );
        Assertions.assertEquals("aaa", CONSUMED_MESSAGE.get());
    }
    ...
    @RocketMQMessageListener(
            topic = "EngineTestDelayTopic",
            consumerGroup = "EngineTestDelayTopic_CG"
    )
